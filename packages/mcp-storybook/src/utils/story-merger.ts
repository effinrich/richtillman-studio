...
export function mergeStoryContent(
  existing: string,
  storySource: string,
  storyName: string,
): string {
  const incoming = storySource.trimEnd() + "\n"
  const existingBlock = extractStoryBlock(existing, storyName)
  if (existingBlock) {
    return existing.replace(existingBlock, incoming)
  }
